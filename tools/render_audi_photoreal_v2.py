...
Output: renders/crypsorender_v01/SHOWCASE_AUDI_PHOTOREAL_v2.png
...
OUT_2K = ROOT / 'renders' / 'crypsorender_v01' / 'SHOWCASE_AUDI_PHOTOREAL_v2_2k.png'
OUT_1K = ROOT / 'renders' / 'crypsorender_v01' / 'SHOWCASE_AUDI_PHOTOREAL_v2.png'
...
def main():
    ...
    KEY_RGB  = np.array([1.0, 0.96, 0.88], dtype=np.float32) * 0.45     # warm key
    FILL_RGB = np.array([0.55, 0.65, 0.85], dtype=np.float32) * 0.18    # cool fill
    RIM_RGB  = np.array([0.95, 0.95, 1.00], dtype=np.float32) * 0.30    # bright neutral rim
    ...
    shaded_hdr = apply_photoreal_lighting(
        ...
        env_ambient_strength=0.08,
        env_reflection_strength=0.20,
    ...
    graded = color_grade(tone, exposure_stops=-0.6, contrast=1.18,
                         saturation=1.20)
