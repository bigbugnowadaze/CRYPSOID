...
# Refactored 2026-04-30 to accept CLI args instead of hardcoded /mnt/data paths.
# Encoding logic below is unchanged from the recovered original.
import os, json, zlib, struct, binascii, shutil, tarfile, zipfile, math, argparse
from pathlib import Path
import numpy as np

_p = argparse.ArgumentParser(description='Build CRYPSOID v0.28 SH-VQ render + q8-exact archive containers from v25 + v27.')
_p.add_argument('--v25', type=Path, required=True, help='Path to v25 .3dphox container')
_p.add_argument('--v27', type=Path, required=True, help='Path to v27 .3dphox container')
_p.add_argument('--v25-report', type=Path, required=True, help='Path to PHOXBENCH_V25_ATTRIBUTE_GROUP_REPORT.json')
_p.add_argument('--v27-report', type=Path, required=True, help='Path to PHOXBENCH_V27_SH_DEBT_REPORT.json')
_p.add_argument('--output-root', type=Path, required=True, help='Root dir; creates outputs/ reports/ tools/ subdirs')
_args = _p.parse_args()
...
V25 = _args.v25
V27 = _args.v27
V25_REPORT = _args.v25_report
V27_REPORT = _args.v27_report
...
# README; the script does not re-package itself or shell out to /mnt/data anymore.
print(json.dumps({'render':render_size,'archive':archive_size,'exact':readback_exact,'crc':crc_ok,'render_path':str(render_path),'archive_path':str(archive_path)},indent=2))
